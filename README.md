...
In Q2 2022, Nervos Network will undergo a hard fork to upgrade the network with new features. This hard fork is non-contentious, meaning there are no parties in opposition to the changes. A complete network migration is expected to occur on **May 1, 2022**.
...
4.  **Omni Lock Migration:** All accounts which utilize PW-Lock-based addresses should be migrated to the new Omni Lock. This new lock is audited and includes all the functionality from PW-Lock. Users will be able to continue using PW-Lock for the time being as PW-Lock addresses have not been formally deprecated yet. However, all projects are encouraged to begin the migration process as soon as possible.
5.  **User Asset Migration:** After switching users to Omni Lock, their assets must be migrated from their old PW-Lock address to their new Omni Lock address.
...
Omni Lock is a new lock script that includes all the interoperability functionality of PW-Lock, and many new and powerful features. Omni Lock has been fully audited and is now recommended as the replacement for PW-Lock which is no longer being maintained.
...
### Omni Lock Documentation

Please see this [blog post](https://blog.cryptape.com/omnilock-a-universal-lock-that-powers-interoperability-1) on the Cryptape Jungle for a detailed explanation of how Omni Lock functions.
